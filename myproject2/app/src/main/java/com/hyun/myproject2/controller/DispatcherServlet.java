...
public class DispatcherServlet extends HttpServlet {
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    String controllerPath = request.getPathInfo(); //ex)/myboard/list
    ...
    try {
      ...
      RequestMappingHandler requestMappingHandler = (RequestMappingHandler) 애플리케이션보관소.getAttribute(controllerPath); //myboard/list
      ...
      //request handler를 호출할 떄 위에서 준비한 파라미터 값을 전달한다.
  ...
  private Object getRequestParameter(HttpServletRequest request, Parameter parameter) {
    //request handler의 파라미터에서 @RequestParam을 추출한다.
    RequestParam anno = parameter.getAnnotation(RequestParam.class);

    //@RequestParam에서 이름을 추출한다.
    ...
    if (parameter.getType() == String.class) {
      ...
      throw new RuntimeException("요청 파라미터 값을 변환할 수 없습니다");
    }
  }
}
