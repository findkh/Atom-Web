...
import com.hyun.myproject2.controller.RequestMappingHandler;
...
public class ContextLoaderListner implements ServletContextListener{
  ...
  public void contextInitialized(ServletContextEvent sce) {
    ...
    try {
      ...
      sc.setAttribute("myboardService", myboardService);

      createBeans("com.hyun.myproject2.controller");
  ...
  private void createBeans(String packageName) throws Exception {
    //packageName: ex) com.hyun.myproject2.controller
  ...
  private void findComponent(File dir, String packageName) throws Exception {
    ...
    for (File f : files) {
      if(f.isDirectory()) {
        ...
        if (compAnno == null) {
          continue; //@Component 애노테이션이 붙은 클래스가 아니라면 다음항목을 반복한다.
        }
