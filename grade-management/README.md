# 📑 성적관리 프로그램 
  ...
  ## 2단계 : 데이터 처리 기능을 분리 (실시간 데이터 공유)
    ...
    - 디버깅
      - 목록을 조회할 때 null이 나옴 -> 서버와 클라이언트가 프로토콜에 따라 보내고 받고의 순서가 맞아야하는데 내가 잘못 작성하였다..
        (컴퓨터는 거짓말을 하지 않는다....😩)
        디버깅하면서 수정사항들을 모두 적어두면서 코드 수정했는데... atom에서 README 파일 작성이 불편해서 VS code로 옮기는 도중... 작성했던 모든 디버깅과 이론들이 날라갔다..
  ...
  ## 3단계 & 4단계 : 멀티스레딩으로 클라이언트의 요청을 동시에 처리, 서버와의 통신 기능을 캡슐화
    ...
    - 4단계 코드 수정
      - 서버쪽에서 scoreTableProxy와 scoreTableException을 작성(*.jar 파일)하여 클라이언트쪽에 준다.
